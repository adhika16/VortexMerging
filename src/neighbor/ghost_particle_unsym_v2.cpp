...
mat neighbor::ghost_particle_unsym_v2(const mat& XP, const mat& index, const mat& f, const mat& u, 
	float maxdom, float mindom, int choice){
	// dat2ghost = join_rows(XP, index, f, u)
	/*
	...
	*/
	...
	dat2ghost 	= join_rows(dat2ghost, f);
	dat2ghost 	= join_rows(dat2ghost, u);
	...
	sz 				= matghost.n_rows;
	float offs 		= 1.2;
	mat dat_temp 	= zeros(1,6);
	mat dat_fit 	= zeros(1,6);
	for (int i = 0; i < sz; i++)
	{
